...
function renderMeasurements(body) {
    ...
    let linkhtml = "";
    try {
        let prevref = body["@controls"].prev.href;
        linkhtml = "<a href='" + prevref + "' onClick='followLink(event, this, renderMeasurements)'>Prev</a> | ";
    } catch(err) {}

    try {
        let nextref = body["@controls"].next.href;
        linkhtml = linkhtml + "<a href='" + nextref + "' onClick='followLink(event, this, renderMeasurements)'>Next</a>";
    } catch(err) {}
...
function appendMeasurementRow(body) {
    $(".resulttable tbody").append(measurementRow(body));
}
